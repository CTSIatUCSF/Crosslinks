...
public class Affiliations {
	...
	private static final Logger LOG = Logger.getLogger(Affiliations.class.getName());
    ...
    public String getAffiliations() {
		...
		try {
			...
			pw.println("<h3>Click and have fun!</h3><p>");
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			pw.println("<ul>");
			while (rs.next()) {
				pw.println("<li>");				
				pw.print("<a href = 'affiliations/" + rs.getString(1) + "'>" + rs.getString(1) + "</a>");
    ...
    public String getAffiliationDetail(@PathParam("affiliation") String affiliation) {
		...
		try {
			...
			while (rs.next()) {
				...
				pw.println("<a href = '" + rs.getString(1) + "/researchers'>Indexed researchers from " + rs.getString(1) + "</a><p>"); 
				...
				pw.println("<a href = '" + rs.getString(1) + "/possibleSamePeople'>List of researchers at other affiliations that we think are also in " + rs.getString(1) + " (CSV Format)</a><p>"); 
				pw.println("<a href = '" + rs.getString(1) + "/possibleConflicts'>List of potential disambiguation conflicts for " + rs.getString(1) + " (CSV format)</a><p>"); 
			}
			pw.println(NOTES);
			pw.println(HTML_END);
			pw.flush();
    ...
    public String getResearchers(@PathParam("affiliation") String affiliation) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs.getInt(7) > 0) {
					pw.println("<a href = '../../researchers/coauthors?authorURL=" + rs.getString(5) + "'>List of " + rs.getInt(7) + " external coauthors and PMID's for " + name + " (CSV Format)</a>&nbsp");
				}
				pw.println("</li>");				
			}
			pw.println("</ul>");
